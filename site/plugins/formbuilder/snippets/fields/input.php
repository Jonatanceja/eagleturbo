...
<?php endif; ?>
<?php if($label):?>
    <label class="uppercase block text-gris-dark text-sm font-bold mb-1 mt-2" for="<?= $name ?>"><?= $label ?></label>
<?php endif; ?>
    <input class="border-gris rounded-md mb-1 w-full md:w-2/3" type="<?= $type ?>" name="<?= $name ?>" id="<?= $name ?>"<?php if(!$useDiv and $class): ?> class="<?= $class ?>"<?php endif; ?> value="<?= $value ?>"<?php if($placeholder): ?> placeholder="<?= $placeholder ?>"<?php endif; ?><?php if($min):?> minlength="<?= $min ?>"<?php endif; ?><?php if($max):?> maxlength="<?= $max ?>"<?php endif; ?><?php if($pattern):?> pattern="<?= $pattern ?>"<?php endif; ?><?php if($req):?> required<?php endif; ?>>
<?php if($useDiv): ?>
</div>
<?php endif; ?>
