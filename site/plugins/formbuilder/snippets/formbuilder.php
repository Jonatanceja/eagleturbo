...
<div<?php if($pg->fb_formbtns_class()->isNotEmpty()): ?> class="<?= $pg->fb_formbtns_class()->html() ?>"<?php endif; ?>>
    <button class="bg-azul px-5 py-2 text-white uppercase rounded-full my-5 transform hover:scale-110 transition group" type="submit" name="submit"><?= $pg->fb_submit_label()->or("Submit")->html() ?></button>
